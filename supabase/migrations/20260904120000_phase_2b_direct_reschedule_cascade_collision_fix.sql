-- Phase 2B Immediate Ops — direct reschedule cascade collision fix
-- When p_cascade=true, exclude the full same-pass cascade set from anchor
-- collision checks so later lessons' old slots do not falsely block postponement.
...
REVOKE ALL ON FUNCTION reve_private.student_has_operational_lesson_collision_excluding(
...
BEGIN
  ...
  -- Default: exclude only the anchor. Cascade: exclude anchor + eligible downstream
  -- lessons that will move in this same transaction (avoids false self-pass collisions).
  v_exclude_ids := ARRAY[v_lesson.id];
  ...
  IF p_cascade THEN
    ...
    SELECT array_agg(l.id ORDER BY l.sequence_number)
    INTO v_exclude_ids
    FROM public.lessons AS l
  ...
  PERFORM reve_private.student_has_operational_lesson_collision_excluding(
    v_lesson.student_id,
    p_new_scheduled_at,
    v_duration,
    v_exclude_ids
...
COMMENT ON FUNCTION public.reve_owner_direct_reschedule_lesson(
  uuid, timestamptz, timestamptz, text, boolean, timestamptz
) IS
  'Owner direct lesson reschedule; optional same-pass cascade excludes moving lessons from collision checks.';
