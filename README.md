...
**Planning & Architecture** — Phase 0B-3B-2B-3A **profile provisioning and owner people master data** verified. Bootstrap is service-role-only; student/teacher records managed by owner RPCs; no passwords stored; physical deletion prohibited. Prior phases through reserved lesson shells (2B-2A) verified. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A ~ 0B-3B-2B-3A)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-3A까지 **애플리케이션 UI가 없습니다.** 프로필 부트스트랩(service_role 전용), 프로필/학생/강사 owner RPC, 결제·회차권 갱신, 수업 상태 전환, safe read RPC, RLS가 적용되어 있습니다. 강좌/상품, 초기 회차권, 환불·일정 승인 등은 미구현입니다.

**로컬 런타임 검증**: `db reset` 9개 마이그레이션 적용, pgTAP **396/396** 통과.
