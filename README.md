...
**Planning & Architecture** — Phase 0B-3B-2B-3B **course and course-product management** verified. Owner RPCs manage courses/products; codes immutable; deactivation preserves history; active dependencies block unsafe deactivation. Prior phases through profile/people (3A) verified. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A ~ 0B-3B-2B-3B)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-3B까지 **애플리케이션 UI가 없습니다.** 프로필 부트스트랩(service_role 전용), 프로필/학생/강사/강좌/상품 owner RPC, 결제·회차권 갱신, 수업 상태 전환, safe read RPC, RLS가 적용되어 있습니다. 초기 회차권 생성, 환불·일정 승인 등은 미구현입니다.

**로컬 런타임 검증**: `db reset` 10개 마이그레이션 적용, pgTAP **465/465** 통과.
