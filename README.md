...
**Planning & Architecture** — Phase 0B-3A core schema and **Phase 0B-3B-1** identity helpers / role-scoped RLS verified locally. Trusted business operations and application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A / 0B-3B-1)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-1까지 **애플리케이션 UI가 없습니다.** RLS는 15개 테이블 모두 **활성화**되어 있으며, **역할별 정책(0B-3B-1)** 과 private identity helper가 적용되어 있습니다. Trusted business operations은 아직 구현되지 않았습니다.

**로컬 런타임 검증 (2026-06-26)**: `db reset` 4개 마이그레이션 적용, pgTAP core schema **60/60** + identity/RLS tests 통과.
