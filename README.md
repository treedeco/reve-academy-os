...
**Planning & Architecture** — Phase 0B-3B-1 identity/RLS and **Phase 0B-3B-2A** safe student/teacher read RPCs verified locally. Base financial and contractual tables remain protected. Business mutation functions and application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A / 0B-3B-1 / 0B-3B-2A)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2A까지 **애플리케이션 UI가 없습니다.** RLS와 private identity helper(0B-3B-1) 및 **safe read RPC**(`reve_get_*`, 0B-3B-2A)가 적용되어 있습니다. 민감 base table 직접 접근은 여전히 차단됩니다. Trusted business mutation functions은 미구현입니다.

**로컬 런타임 검증**: `db reset` 5개 마이그레이션 적용, pgTAP **218/218** 통과.
