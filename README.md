...
**Planning & Architecture** — Phase 0B-3B-2B-3D-2B **optional cascade rescheduling and SMS sync correction** verified (latest **database** checkpoint). Phase 0B-3B-2B-3D-3A **Owner manual SMS sent confirmation specification** is the current **documentation** checkpoint; Phase 0B-3B-2B-3D-3B (database RPC) is next. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A ~ 0B-3B-2B-3D-2B)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-3D-2B까지 **애플리케이션 UI가 없습니다.** 일정 변경 검토·적용·연쇄 재배치 owner RPC, SMS 동기화, pass 시간표 교체, 결제·회차권 갱신, safe read RPC, RLS가 적용되어 있습니다. Owner SMS 발송 확인(`confirm_sms_sent`)은 **3D-3A 사양만 확정**, DB 구현(3D-3B)은 미착수. 환불 등은 미구현입니다.

**로컬 런타임 검증**: `db reset` 14개 마이그레이션 적용, pgTAP **827/827** 통과.

## Phase 0B-3B-2B-3D-3 (문서 — 3D-3A)

**Owner manual SMS sent confirmation** — Owner가 외부 SMS 앱으로 수동 발송 후 시스템에 발송 완료를 기록하는 trusted operation. 3D-3A는 canonical contract 문서화만; 3D-3B에서 RPC·pgTAP 구현 예정. Owner UI는 deferred.
