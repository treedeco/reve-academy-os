...
**Planning & Architecture** — Phase 0B-3B-2B-3D-2A **schedule-change review and single-lesson rescheduling** verified. Review and apply are separate; fixed timetable unchanged; cascade deferred. Prior phases through pass schedule management (3D-1) verified. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A ~ 0B-3B-2B-3D-2A)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-3D-2A까지 **애플리케이션 UI가 없습니다.** 일정 변경 요청 검토·적용 owner RPC, pass 시간표 교체, 초기 등록, 결제·회차권 갱신, safe read RPC, RLS가 적용되어 있습니다. 연쇄 일정 변경, 환불 등은 미구현입니다.

**로컬 런타임 검증**: `db reset` 13개 마이그레이션 적용, pgTAP **721/721** 통과.
