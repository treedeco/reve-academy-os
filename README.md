...
**Planning & Architecture** — Phase 0B-3B-2B-3C **initial enrollment** verified. Atomic first payment, pass sequence 001, fixed schedule snapshot, lesson generation, collision rollback, and idempotency. Prior phases through course/product (3B) verified. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A ~ 0B-3B-2B-3C)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-3C까지 **애플리케이션 UI가 없습니다.** 초기 등록 owner RPC, 프로필/강좌/상품 owner RPC, 결제·회차권 갱신, 수업 상태 전환, safe read RPC, RLS가 적용되어 있습니다. 일반 일정 편집, 환불·일정 승인 등은 미구현입니다.

**로컬 런타임 검증**: `db reset` 11개 마이그레이션 적용, pgTAP **550/550** 통과.
