...
**Planning & Architecture** — Phase 0B-3B-2B-2A **reserved lesson-shell compliance** verified. Payment completion creates all lesson rows; reserved shells have pending dates finalized at activation. Prior phases: payment renewal (2B-2), lesson transitions (2B-1), safe reads (2B-2A), identity/RLS (3B-1), core schema (3A). Application UI **not** started.
...
**로컬 런타임 검증**: `db reset` 8개 마이그레이션 적용, pgTAP **341/341** 통과.
