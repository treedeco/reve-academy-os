...
**Planning & Architecture** — Phase 0B-2 complete; **Phase 0B-3A** core schema migrated and locally verified (pgTAP 60/60). Role-specific RLS policies pending Phase 0B-3B. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3A에는 **애플리케이션 UI가 없습니다.** RLS는 15개 테이블 모두 **활성화**(default deny)되어 있으나, **역할별 정책은 Phase 0B-3B**에서 추가됩니다.

**로컬 런타임 검증 (2026-06-26)**: `db reset` 3개 마이그레이션 적용 성공, pgTAP **60/60** 통과.
