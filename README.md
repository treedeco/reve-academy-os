...
**Planning & Architecture** — Phase 0B-3B-2B-1 **trusted lesson status transitions** verified locally. Safe read RPCs (0B-3B-2A), identity/RLS (0B-3B-1), and core schema (0B-3A) precede this checkpoint. Usage and SMS display values remain **derived or synchronized from lesson status**; reserved-pass activation remains **deferred**. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A / 0B-3B-1 / 0B-3B-2A / 0B-3B-2B-1)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-1까지 **애플리케이션 UI가 없습니다.** RLS, safe read RPC(`reve_get_*`), **lesson transition RPC**(`reve_transition_lesson_status`, `reve_correct_lesson_status`)가 적용되어 있습니다. 민감 base table 직접 접근은 여전히 차단됩니다. 결제 갱신, 회차권 생성, 예약 회차권 활성화, 환불, 일정 연쇄 변경 등 나머지 trusted mutation은 미구현입니다.

**로컬 런타임 검증**: `db reset` 6개 마이그레이션 적용, pgTAP **280/280** 통과.
