...
**Planning & Architecture** — Phase 0B-3B-2B-2 **payment completion, pass renewal, and reserved-pass activation** verified locally. Lesson transitions (0B-3B-2B-1), safe read RPCs (0B-3B-2A), identity/RLS (0B-3B-1), and core schema (0B-3A) precede this checkpoint. Early renewal creates a **reserved** pass when the current pass still has remaining lessons; the **final deductible lesson auto-activates** the reserved pass. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A / 0B-3B-1 / 0B-3B-2A / 0B-3B-2B-1 / 0B-3B-2B-2)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-2까지 **애플리케이션 UI가 없습니다.** 결제 완료·회차권 갱신(`reve_complete_payment_and_renew_pass`), 예약 회차권 활성화(`reve_activate_reserved_pass`), 수업 상태 전환 RPC, safe read RPC, RLS가 적용되어 있습니다. 환불·일정 승인·연쇄 변경 등은 미구현입니다.

**로컬 런타임 검증**: `db reset` 7개 마이그레이션 적용, pgTAP **327/327** 통과.
