...
**Planning & Architecture** — Phase 0B-3B-2B-3D-3B **Owner manual SMS sent confirmation** verified (latest **database** checkpoint). Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A ~ 0B-3B-2B-3D-3B)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-3D-3B까지 **애플리케이션 UI가 없습니다.** Owner SMS 발송 확인 RPC(`reve_owner_confirm_sms_sent`), 일정 변경·연쇄 재배치, SMS 동기화, pass 시간표 교체, 결제·회차권 갱신, safe read RPC, RLS가 적용되어 있습니다. Owner SMS 확인 **UI** 및 **외부 SMS API**는 deferred. 환불 등은 미구현입니다.

**로컬 런타임 검증**: `db reset` 15개 마이그레이션 적용. pgTAP **854** (13 files) + concurrency harness **1** = **855** total.

```powershell
npx supabase start
npx supabase db reset
npx supabase db lint
# Run pgTAP except the z_ concurrency assertion file (854 tests)
Get-ChildItem supabase/tests/database/*.test.sql |
  Where-Object { $_.Name -notlike '*_z_*' } |
  ForEach-Object { npx supabase test db $_.FullName }
powershell -ExecutionPolicy Bypass -File scripts/verify_sms_concurrency.ps1
npx supabase test db supabase/tests/database/phase_0b3b2b3d3b_z_owner_sms_concurrency.test.sql
```
