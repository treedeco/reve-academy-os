...
**Planning & Architecture** — Phase 0B-3B-2B-3E **Owner payment refund** verified (latest **database** checkpoint). Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A ~ 0B-3B-2B-3E)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-3E까지 **애플리케이션 UI가 없습니다.** Owner SMS 발송 확인, Owner 결제 환불(`reve_process_payment_refund`), 일정 변경·연쇄 재배치, SMS 동기화, pass 시간표 교체, 결제·회차권 갱신, safe read RPC, RLS가 적용되어 있습니다. Owner **UI**, **외부 SMS API**, `correct_cancelled_pass`, re-enrollment는 deferred.

**로컬 런타임 검증**: `db reset` 17개 마이그레이션 적용. 표준 pgTAP **882**. SMS concurrency pgTAP **1** + refund concurrency pgTAP **2** (별도 스크립트).
