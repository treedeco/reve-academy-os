...
**Planning & Architecture** — Phase 0B-3B-2B-3D-1 **pass schedule-slot replacement** verified. Fixed timetable changes do not move lesson dates; historical inactive slots preserved. Prior phases through initial enrollment (3C) verified. Application UI **not** started.
...
## 로컬 데이터베이스 (Phase 0B-3A ~ 0B-3B-2B-3D-1)

**요구 사항**: **Docker Desktop**(엔진 실행 중), Node.js, Supabase CLI (`npx supabase`).

Phase 0B-3B-2B-3D-1까지 **애플리케이션 UI가 없습니다.** pass 고정 시간표 교체 owner RPC, 초기 등록, 프로필/강좌/상품 owner RPC, 결제·회차권 갱신, 수업 상태 전환, safe read RPC, RLS가 적용되어 있습니다. 수업 일정 연쇄·연기, 환불·일정 승인 등은 미구현입니다.

**로컬 런타임 검증**: `db reset` 12개 마이그레이션 적용, pgTAP **635/635** 통과.
