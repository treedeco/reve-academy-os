...
Executable PostgreSQL migrations and pgTAP tests. **Current database checkpoint**: Phase 0B-3B-2B-3D-2B (tag `phase-0b3b2b3d2b-lesson-cascade-rescheduling`). **Current documentation checkpoint**: Phase 0B-3B-2B-3D-3A.
...
| 0B-3B-2B-3D-3 | Owner manual SMS sent confirmation | **In progress** |
| 0B-3B-2B-3D-3A | SMS sent confirmation specification (docs only) | **Current** |
| 0B-3B-2B-3D-3B | SMS sent confirmation database RPC + pgTAP | **Next** |
...
- **3D-3B** — Owner-only trusted RPC (`confirm_sms_sent`), database authorization, atomic transition, idempotency, concurrency, audit, pgTAP tests.
