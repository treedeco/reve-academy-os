...
test.describe('Owner fixed-schedule re-register with overlap warning-only', () => {
  test.beforeEach(() => {
    applySqlFixture('fixture-fixed-schedule-reregister.sql');
  });

  test('saves overlapping fixed schedule without blocking confirmation', async ({ page }) => {
    ...
    await page.getByTestId('schedule-change-reason').fill('E2E overlap warning-only save');
    await page.getByTestId('schedule-change-next').click();
    await expect(page.getByTestId('schedule-overlap-soft-warning')).toBeVisible({ timeout: 10_000 });
